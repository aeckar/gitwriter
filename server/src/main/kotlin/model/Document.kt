...
data class Document(
    ...
    val repo: Repository,
    val homePath: String,
    val contentDirectory: Directory
) {
    ...
    suspend operator fun get(path: String): String {
        ...
        val file = content(repo, path.ifEmpty { homePath }).orResourceNotFound { "Page '$path' not found" }
    ...
    companion object {
        ...
        suspend fun get(repo: Repository): Document {
            ...
            val config = content(repo, "site.toml").orResourceNotFound { "Repository '/$repo' not found" }
            val document = Toml.Default
